...
<template>
  <div
    class="flex flex-col h-screen w-screen overflow-hidden bg-stone-900 text-white bg-[url('/images/wood.jpg')] bg-cover bg-center font-asap"
  >
    <header
      class="group trans flex-none h-7 flex items-center justify-between px-6 z-20 opacity-40 hover:opacity-100 bg-black/80 backdrop-blur-lg fixed top-0 w-full left-0 hover:bg-black/90 border-b border-white/5"
    >
      <div
        class="trans font-asap font-bold italic text-sm group-hover:text-amber-500 w-32 tracking-wider flex items-center gap-2"
      >
    ...
    <main
      class="flex-grow w-full h-full flex items-center justify-center overflow-hidden relative pt-10"
    >
    ...
    <div
      v-if="store.gameStatus === 'finished'"
      class="fixed inset-0 bg-black/90 backdrop-blur-md flex items-center justify-center z-50"
    >
      <div
        class="bg-stone-800 p-10 rounded-2xl border border-stone-600 shadow-2xl text-center max-w-md w-full mx-4"
      >
        ...
        <div class="space-y-3 mb-8 text-left text-base" v-if="!store.isFreePlay">
          <div
            v-for="p in store.players"
            :key="p.id"
            class="flex justify-between border-b border-stone-700 pb-3 items-end"
          >
            <span class="text-stone-300 font-asap">{{ p.name }}</span>
            <div class="text-right">
              <span class="block font-bold text-yellow-500 text-xl font-asap italic"
                >{{ p.score }} bodů</span
              >
        ...
        <div class="mb-8 p-4 bg-black/40 rounded-lg text-center">
          <span class="text-xs text-stone-500 uppercase tracking-widest block mb-1"
            >Celkový čas</span
          >
          <span class="text-2xl font-asap font-bold italic text-white">{{
            store.formattedDuration
          }}</span>
        </div>
